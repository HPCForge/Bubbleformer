...
This script automates running the generate_moe_similarity_heatmaps.py script on multiple model weights.
It generates similarity heatmaps for each model and saves them to a visualization directory.

Usage:
    python run_moe_visualizations.py
"""
...
def run_visualization(weights_path, dataset, time_window):
    """Run the visualization script for a single weights path"""
    model_dir = extract_model_dir(weights_path)
    output_dir = os.path.join(model_dir, "visualization")
    
    print(f"\n\n==== Running visualization for {os.path.basename(model_dir)} ====\n")
    ...
    cmd = [
        "python", 
        "scripts/generate_moe_similarity_heatmaps.py", 
    ...
    print(f"Visualization completed for {os.path.basename(model_dir)}")
    print(f"Outputs saved to {output_dir}")

def main():
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description='Generate MoE similarity heatmaps for multiple models')
    ...
    # Run visualizations for each weights path and dataset
    for weights_path in weights_paths:
        for dataset in datasets:
            try:
                run_visualization(weights_path, dataset, args.time_window)
            except Exception as e:
                print(f"Error running visualization for {weights_path} on {dataset}: {e}")
