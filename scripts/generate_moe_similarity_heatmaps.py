...
class MoEHeatmapGenerator:
    """
    Utility class to generate heatmaps showing cosine similarity
    between expert outputs at each spatial position and a reference position.
    """
    ...
    def generate_similarity_heatmaps(self, batch_size=1, time_window=1):
        """
        ...
        for layer_idx, layer in enumerate(self.moe_layers):
            ...
            plt.tight_layout()
            fig.suptitle(f"Layer {layer_idx}: {self.layer_names[layer_idx]} - Expert Output Similarity", fontsize=16)
...
def main():
    parser = argparse.ArgumentParser(description='Generate MoE similarity heatmaps')
    ...
    parser.add_argument('--output_dir', type=str, default='./moe_heatmaps', help='Output directory for heatmaps')
    ...
    heatmap_generator = MoEHeatmapGenerator(model)
    ...
    figures = heatmap_generator.generate_similarity_heatmaps(1, args.time_window)
    
    # Save heatmaps
    os.makedirs(args.output_dir, exist_ok=True)
    for i, fig in enumerate(figures):
        output_path = os.path.join(args.output_dir, f"moe_layer_{i}_similarity_{args.dataset}.png")
        fig.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Saved heatmap to {output_path}")
    
    print(f"Generated {len(figures)} heatmaps")
