...
from bubbleformer.data.batching import make_data, VariableBatch
...
def collate_random_variable(
    ...
    if has_fluid:
        ...
        return VariableBatch(input=x, target=y, fluid_params_tensor=fluids)

    return VariableBatch(input=x, target=y)
